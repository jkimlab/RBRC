...
$cmd = "$in_assembler_path -1 $out_dir/cluster.out/$pre_cluster.1.fq -2 $out_dir/cluster.out/$pre_cluster.2.fq -t $in_cpu -o $out_dir/cluster.out/$pre_cluster.out 2> $out_dir/logs/log.$pre_cluster.txt";
print "$cmd\n";
print "$Bin/merging_unicycler_incluster.pl $out_dir/cluster.out $out_dir\n";;
$cmd = "$in_assembler_path -1 $in_read1 -2 $in_read2 -l $out_dir/contigs_incluster.fa -t $in_cpu -o $out_dir/Final_assembly 2> $out_dir/logs/log.Final_assembly.txt";
print "$cmd\n";
